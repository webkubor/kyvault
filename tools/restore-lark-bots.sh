...
for bot in "${WANT[@]}"; do
  # 平台可能是 feishu 或 lark
  plat=feishu
  cs kyvault get "secret://feishu/${bot}-app-id" >/dev/null 2>&1 || plat=lark
  id=$(cs kyvault get "secret://${plat}/${bot}-app-id" 2>/dev/null | tail -1)
  sec=$(cs kyvault get "secret://${plat}/${bot}-app-secret" 2>/dev/null | tail -1)
